...
std::string CacheManager::versionTableName(ETableType type)
{
	...
	case TYPE_HISTORY_IMAGE:
		return "THistoryImageVersion";

	case TYPE_HISTORY_IMAGE_DATE:
		return "THistoryImageDateStringVersion";

	case TYPE_HISTORY_IMAGE_INFO:
		return "THistoryImageInfoVersion";
...
std::string CacheManager::faltFileTableName(ETableType type)
{
	...
	case TYPE_HISTORY_IMAGE:
		return "THistoryImageFaltfile";

	case TYPE_HISTORY_IMAGE_DATE:
		return "THistoryImageDateString";

	case TYPE_HISTORY_IMAGE_INFO:
		return "THistoryImageInfo";
...
bool CacheManager::AddImageDates(const std::string& name, const std::string data, ETableType type)
{
	if (m_pSqlite == NULL)
		return false;
	...
	{
		std::stringstream ssSQL;
		ssSQL << "SELECT url FROM " << tableName << " WHERE url='" << name << "'";
		sqlite3_stmt* stmt = SQLExec(ssSQL.str().c_str(), nullptr);
		if (stmt != nullptr)
		{
			sqlite3_finalize(stmt);
...

	{
		...
		sqlite3_bind_text(pStmt, 1, name.c_str(), name.size(), SQLITE_STATIC);
...
bool CacheManager::AddHistoryImageVersionInfo(const std::string& name, const std::string data, ETableType type)
{
	if (m_pSqlite == NULL)
		return false;
...
std::string CacheManager::GetHistoryImageVersionInfo(const std::string& name, ETableType type){
	std::string data;
	if (m_pSqlite == NULL)
		return data;
	...
	std::stringstream ssSQL;
	ssSQL << "SELECT data FROM " << tableName << " WHERE url='" << name << "'";
	sqlite3_stmt* stmt = SQLExec(ssSQL.str().c_str(), nullptr);
	if (stmt != nullptr)
	{
...
std::string CacheManager::GetImageDates(const std::string& name, ETableType type)
{
	std::string data;
	if (m_pSqlite == NULL)
		return data;
