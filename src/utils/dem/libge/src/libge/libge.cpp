...
QuadTreePacket16* CLibGEHelper::DecodeTmQuadtree(const char* srcData, unsigned long srcSize)
{
	if (srcData == nullptr || srcSize <= 0)
		return nullptr;

	QuadTreePacket16* packet = new QuadTreePacket16();
	if (!packet->decodeTmQtree(srcData, srcSize))
	{
		delete packet;
		return nullptr;
	}
...
bool CLibGEHelper::getTmDBRoot()
{
	...
	unsigned short _unk;
	convertEndian((LPVOID)(data + 4), 2, &_unk, true);
	convertEndian((LPVOID)(data + 6), 2, &_tm_version, true);
	data += 8;
	memset(_crypt_key, 0, sizeof(_crypt_key));
	memcpy(_crypt_key + 8, data, 1016);
	data += 1016;
	...
	if (!strXML.empty())
	{
		...
		{
			std::stringstream ssEncodePath;
			ssEncodePath << _cachePath << "\\tm_dbroot5.xml";
...
std::set<std::string> CLibGEHelper::getImageDates(const char* name, CacheManager::ETableType type)
{
	if (_tm_version <= 0)
		getTmDBRoot();

	std::set<std::string> images_dates;
	...
	//
	std::string dates = CacheManager::GetInstance().GetImageDates(qtName, type);
	if (dates.empty()) {
		QuadTreePacket16* packet = getTmQuadtree(qtName.c_str(), 0);
		if (packet == nullptr)
			return images_dates;
		images_dates = packet->_quadtree_packet_protobuf._imagery_dates;
		std::string strAllImageDates = setToStr(images_dates);
		CacheManager::GetInstance().AddImageDates(qtName, strAllImageDates, type);
		//
		auto &map_of_history_images = packet->_quadtree_packet_protobuf._map_of_history_images;
		std::string history_image_version_info;
		for (auto &m : map_of_history_images) {
			std::string full_key = qtName + m.first;
			for (auto &v : m.second){
				history_image_version_info += full_key + "," + v.image_date + "," + v.image_date_hex + "," + v.image_version + "\n";
			}
		}
		CacheManager::GetInstance().AddHistoryImageVersionInfo(qtName, history_image_version_info, CacheManager::ETableType::TYPE_HISTORY_IMAGE_INFO);
		//
		delete packet;
	}
	else {
		images_dates = strToSet(dates);
	}
	//
	return images_dates;
}

std::string CLibGEHelper::getHistoryImageInfo(const char* name, CacheManager::ETableType type)
{
	if (_tm_version <= 0)
		getTmDBRoot();
	...
	//
	std::string history_image_info = CacheManager::GetInstance().GetHistoryImageVersionInfo(qtName, type);
	if (history_image_info.empty()) {
		QuadTreePacket16* packet = getTmQuadtree(qtName.c_str(), 0);
		if (packet == nullptr)
			return history_image_info;
		//
		auto &map_of_history_images = packet->_quadtree_packet_protobuf._map_of_history_images;
		for (auto &m : map_of_history_images) {
			std::string full_key = qtName + m.first;
			for (auto &v : m.second){
				history_image_info += full_key + "," + v.image_date + "," + v.image_date_hex + "," + v.image_version + "\n";
			}
		}
		CacheManager::GetInstance().AddHistoryImageVersionInfo(qtName, history_image_info, CacheManager::ETableType::TYPE_HISTORY_IMAGE_INFO);
		//
		delete packet;
	}
	//
...
QuadTreePacket16* CLibGEHelper::getTmQuadtree(const char* baseGEName, int version)
{
	if (version <= 0 && _tm_version <= 0)
	{
		if (!getTmDBRoot() || _tm_version <= 0)
			return nullptr;
	}
	std::stringstream ssUrl;
	std::stringstream ssKey;
	...
	QuadTreePacket16* packet = DecodeTmQuadtree(strResponse.c_str(), strResponse.size());
	if (packet == nullptr)
		return nullptr;
...
std::string CLibGEHelper::getHistoryImage(const char* name, const std::string &version, const std::string &date_hex, bool is_mercator)
{
	std::stringstream ssUrl;
	std::stringstream ssKey;
...
std::string CLibGEHelper::setToStr(const std::set<std::string> &input) {
	std::string res;
	for (auto &date : input) {
		if (date == "0001:01:01") continue;
		if (!res.empty()) res += ",";
		res += date;
	}
	return res;
...
std::string CLibGEHelper::getHistoryImageByDates(double minX, double minY, double maxX, double maxY, unsigned int level, const std::string &date){
	return getHistoryImageByDates(minX, minY, maxX, maxY, level, date, 256, 256, false);
}

std::string CLibGEHelper::getHistoryImageByDates(double minX, double minY, double maxX, double maxY, unsigned int level, const std::string &date, unsigned int rasterXSize, unsigned int rasterYSize, bool is_mercator) {
	std::vector<std::string> names = ConvertToQtNode(minY, minX, maxY, maxX, level, is_mercator);
	//std::cout << names.size() << std::endl;
	if (names.size() <= 0)
		return "";
	...
	{
		std::string name = names.at(i);
		std::string history_img_info = getHistoryImageInfo(name.c_str(), CacheManager::ETableType::TYPE_HISTORY_IMAGE_INFO);
		//std::cout << history_img_info << std::endl;
		//
		std::string::size_type pos;
		std::string pattern = "\n";
		std::string pattern1 = ",";
		int size = history_img_info.size();
		std::vector<std::string> vec_histoy_img_infos;
		for (int i = 0; i < size; i++)
		{
			pos = history_img_info.find(pattern, i);
			if (pos < size)
			{
				std::string s = history_img_info.substr(i, pos - i);
				vec_histoy_img_infos.push_back(s);
				i = pos + pattern.size() - 1;
			}
		}
		//
...
std::string CLibGEHelper::getHistoryImageDates(double minX, double minY, double maxX, double maxY, unsigned int level, unsigned int rasterXSize, unsigned int rasterYSize, bool is_mercator)
{
	...
	std::string strAllImageDates;
	std::set<std::string> allImageDates;
	for (int i = 0; i < names.size(); i++)
	{
		std::string name = names.at(i);
		auto imgDates = getImageDates(name.c_str(), CacheManager::ETableType::TYPE_HISTORY_IMAGE_DATE);
		allImageDates.insert(imgDates.begin(), imgDates.end());
	}
	//
	strAllImageDates = setToStr(allImageDates);
	return strAllImageDates;
}

std::string CLibGEHelper::getHistoryImageDates(double minX, double minY, double maxX, double maxY, unsigned int level) {
	return getHistoryImageDates(minX, minY, maxX, maxY, level, 256, 256, false);
}
...
std::string CLibGEHelper::getTerrain(double minX, double minY, double maxX, double maxY, unsigned int level) {
	return getTerrain(minX, minY, maxX, maxY, level, 256, 256, false);
}
